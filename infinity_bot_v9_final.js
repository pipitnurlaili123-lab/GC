/* ============================================
   ✦ INFINITY BOT — WhatsApp Edition V9
   XR Engine + All GC Features
...
function startSpam(socks, chatId, spamText) {
  ...
  const workers = socks.map(sock => makeWorker(async () => {
    // 5 messages at once per bot — ultra fast
    const burst = [];
    for (let i = 0; i < 5; i++) {
      burst.push(sock.sendMessage(chatId, { text: spamText }).catch(() => {}));
    }
    await Promise.all(burst);
    await sleep(10);
  }));
...
async function startBot(botId) {
  ...
  // ── AUTO PAIRING CODE for new (unregistered) sessions ──
  if (!state.creds.registered) {
    const rl2 = readline.createInterface({ input: process.stdin, output: process.stdout });
    rl2.question(`\n📱 BOT ${botId} — Enter phone number (with country code, no +):\n> `, async (phone) => {
      rl2.close();
      ...
      try {
        ...
        console.log(`\n╔══════════════════════════════╗`);
        console.log(`║  🔑 PAIRING CODE BOT ${botId}: ${fmt}  ║`);
        console.log(`╚══════════════════════════════╝`);
...
if (_savedCount) {
  ...
  rl.question(`\n✦ INFINITY BOT V9 ✦\nHow many bots? : `, n => {
    const c = parseInt(n);
    if (!c || c < 1) { console.log("❌ Invalid."); process.exit(1); }
    rl.close();
    launchBots(c);
  });
}
